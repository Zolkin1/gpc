...
def train(  # noqa: PLR0915 this is a long function, don't limit to 50 lines
    env: TrainingEnv,
    sampling_ctrl: SamplingBasedController,
    net: nnx.Module,
    ...
    _spc_params = sampling_ctrl.init_params()
    assert hasattr(
        _spc_params, "mean"
    ), f"Controller '{type(sampling_ctrl).__name__}' is not compatible with GPC."
    ...
    num_samples = num_policy_samples + sampling_ctrl.num_samples
    print("Training with:")
    print(
    ...
    print(
        ...
        f" {num_samples * sampling_ctrl.num_randomizations * num_envs}"
        f" (= {num_samples} x {sampling_ctrl.num_randomizations} x {num_envs})"
    )
    print("")
    ...
    ctrl = PolicyAugmentedController(sampling_ctrl, num_policy_samples)
    ...
    def jit_simulate(
        ...
        Returns:
            ...
            Costs of SPC's best action sequence.
            Costs of the policy's best action sequence.
    ...
    if (with_value):
        # value_fcn = Fvi(env.observation_size, 1000, 2000)
        value_fcn = Fvi(env.observation_size, 100, 2000)

    for i in range(num_iters):
        if (with_value):
            # TODO: Debug/verify
            # Make a new controller with the current value function
            env.task.update_value_fcn(value_fcn=value_fcn)
            sampling_ctrl.task = env.task
            ctrl = PolicyAugmentedController(sampling_ctrl, num_policy_samples)
            assert env.task == ctrl.task
        ...
        if (with_value):
            # Fit the value function w/ the new data
            value_fcn.set_data(y, jnp.minimum(J_spc, J_policy))     # Update the training data
            value_loss = value_fcn.fit()                            # Fit
        ...
        print(
            f"  {i+1}/{num_iters} |"
            f" policy cost {jnp.mean(J_policy):.4f} |"
            f" spc cost {jnp.mean(J_spc):.4f} |"
        ...
        # Tensorboard logging
        tb_writer.add_scalar("sim/policy_cost", jnp.mean(J_policy), i)
        tb_writer.add_scalar("sim/spc_cost", jnp.mean(J_spc), i)
    ...
    # theta = jnp.arctan2(value_fcn.obs[:, :, 1], value_fcn.obs[:, :, 0]).reshape(-1)
    # thetadot = value_fcn.obs[:, :, 2].reshape(-1)
    # print(theta.shape)
    # print(thetadot.shape)
    # Z = value_fcn.targets[:, :].reshape(-1)
    # print(Z.shape)
    #
